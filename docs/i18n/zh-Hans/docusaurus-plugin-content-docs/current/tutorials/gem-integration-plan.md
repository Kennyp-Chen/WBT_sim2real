...
| 2026-08-14 | GEM-007/009 | `Go.mp3` 17.345 秒录制音频 demo | `gem_audio_motion_stream.py` -> SMPL-X global mesh 渲染 -> G1/PiPlus GMR -> BFM-Zero 和 SONIC ZMQ 录制 -> wall-clock 校正及三栏音频封装 | `outputs/gem_retarget/music_go/final/go_human_{g1_reference_bfmzero,piplus_reference_bfmzero,g1_reference_sonic}_with_audio.mp4` | 生成人体 521 帧/30 Hz，机器人 reference 868 帧/50 Hz；全部重定向关节越限为 0；三个 policy 在 2/8/15 秒均保持直立；最终布局为人体 / 机器人 reference / policy，三段均为 1920x720、17.345 秒且带 AAC 音轨；G1 BFM-Zero 和 SONIC 为 30 fps，PiPlus 为保证实时同步使用 20 fps |
